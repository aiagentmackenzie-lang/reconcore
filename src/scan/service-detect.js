 ...
 * Note: group 1 targets the actual version/product string.
 * For SSH, group 1 is the software version after the protocol version.
 * For HTTP, group 1 is the Server header value (contains version info).
 * For FTP/SMTP, group 2 contains the product/version after the status code.
 * @private
 */
function parseVersion(banner, service) {
  ...
  if (pattern) {
    ...
    if (match) {
      // For SSH, compose full version string "protocol software"
      if (service === 'SSH' && match[1] && match[2]) {
        return `${match[1]} ${match[2]}`;
      }
      // For all others, return the first meaningful capture group (group 1)
