...
async function auditHttpHeaders(host, openPorts, options = {}) {
  ...
  // Filter for HTTP and HTTPS ports — security headers must be checked on TLS endpoints too
  const httpPorts = openPorts.filter(p =>
    ['HTTP', 'HTTP-Alt', 'HTTPS', 'HTTPS-Alt'].includes(p.service) ||
    [80, 8080, 443, 8443].includes(p.port)
  );
...
/**
 * Calculate security score based on required headers only.
 * Only required headers count toward the denominator;
 * optional headers (like x-xss-protection) are not penalised
