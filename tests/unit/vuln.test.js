...
describe('Vuln Module', () => {
  ...
  describe('calculateSeverity', () => {
    ...
    test('should not return NONE for fractional scores in range gaps (8.95)', () => {
      expect(calculateSeverity(8.95).label).toBe('HIGH');
      expect(calculateSeverity(6.95).label).toBe('MEDIUM');
      expect(calculateSeverity(3.95).label).toBe('LOW');
    });

    test('should handle boundary values correctly', () => {
      expect(calculateSeverity(9.0).label).toBe('CRITICAL');
      expect(calculateSeverity(10.0).label).toBe('CRITICAL');
      expect(calculateSeverity(7.0).label).toBe('HIGH');
      expect(calculateSeverity(4.0).label).toBe('MEDIUM');
      expect(calculateSeverity(0.1).label).toBe('LOW');
    });
  });
